...
def parse_arguments():
    ...
    verbosity.add_argument(
        "-v", "--verbose", action="count", help="set verbosity level"
    )

    parser.add_argument(
        "-i", "--input", required=True, type=str, help="Path to pos input file"
    )
    parser.add_argument(
        "-o", "--output", required=True, type=str, help="Path to cpl outputput file"
    )
    parser.add_argument(
        "-a", "--adjust", type=str, help="Path to angle adjustment file"
    )
...
def configure_logging(verbose, quiet):
    logConfig(
        ...
        datefmt="%Y-%m-%d %H:%M:%S",
        level=get_log_level(verbose, quiet),
    )
...
def main():
    ...
    with open(args.input, "r", newline="") as in_file, open(
